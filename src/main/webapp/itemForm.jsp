									...
									<form:form method="POST" action="save-vrste-artikala.html"
										...
										<div class='form-group'>
											...
											<form:textarea type="text" class="form-control" id="remark"
												path="remark" value="${vrsteArtikala.remark}" />
											<p class="help-block">
												<form:errors path="remark" class="help-block" />
											</p>
										</div>

										<c:out value="${model.vrsteArtikala.aktivan}" />

										<c:if test="${vrsteArtikala.aktivan}">
											<c:out value="${vrsteArtikala.code}" />
										</c:if>
										...
										<c:if test="${vrsteArtikala.aktivan == true}">
											<c:set var="salary" scope="session" value="checked" />
											<c:set var="salary1" scope="session" value="" />
										</c:if>

										<c:if test="${vrsteArtikala.aktivan == false}">
